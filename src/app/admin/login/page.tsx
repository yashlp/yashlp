  ...
  return (
    <div className="aesthetics-root flex min-h-dvh items-center justify-center bg-[var(--aes-ivory)] px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={adminId ? handleOtp : handleLogin} className="aes-card space-y-4 p-8">
          {!adminId ? (
            <>
              <div>
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-[var(--aes-charcoal-muted)]">
                We sent a one-time code to your admin email. In local dev, check server logs if using demo OTP.
              </p>
              <button
                type="button"
                className="mt-3 text-xs text-[var(--aes-ink-muted)] underline"
                onClick={() => {
                  setAdminId(null);
                  setOtp("");
                  setError("");
