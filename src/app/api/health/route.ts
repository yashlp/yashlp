...
import { getAreaHealthScore, getAreaHighlights } from "@/lib/health-score";
...
export async function GET(req: Request) {
  ...
  const [health, highlights] = await Promise.all([
    getAreaHealthScore(lat, lng, radius),
    getAreaHighlights(lat, lng, radius, 4),
  ]);

  return jsonWithCache({ ...health, ...highlights }, CACHE_PUBLIC_SHORT);
}
