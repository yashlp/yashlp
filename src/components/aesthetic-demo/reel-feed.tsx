...
export function ReelFeed() {
  ...
  const handleSwipe = useCallback(
    (product: AestheticProduct, action: "like" | "pass") => {
      setPrefs((p) => recordSwipe(p, product, action));
      if (action === "like") {
        setCart((c) => [...c, product]);
  ...
  return (
    <div className="relative h-dvh w-full overflow-hidden bg-[#fffef7]">
      ...
      <div
        ref={scrollRef}
        className="aesthetic-reel-scroll h-full overflow-y-auto pt-[max(8.5rem,calc(env(safe-area-inset-top)+7.5rem))]"
      >
        {feed.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center px-8 text-center">
            ...
            <p className="aesthetic-mono mt-2 text-xs text-[#4a7cbb]/70">
              Algorithm learned {topVibes.join(", ") || "your taste"}. Check your cart or reset filters.
            </p>
            <button
              type="button"
              onClick={() => {
              ...
              className="aesthetic-mono mt-6 rounded-full bg-[#1b3a6b] px-6 py-3 text-xs uppercase tracking-wider text-[#fffef7]"
            >
              Reset feed
            </button>
          </div>
